...
- [ ] `alert/`: Alert schema output + SHA-256 hash-chained logging. Test: log a sequence of alerts, verify hash chain integrity (tampering with one entry breaks verification).
- [ ] `drift/`: rolling-window score monitor + KS-test/PSI drift check + bounded retrain trigger + fallback-to-original-baseline logic. Test: simulate a shifted score distribution, assert drift is detected and retrain is bounded/logged.
